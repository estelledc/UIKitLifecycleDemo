...
一个纯 UIKit、纯代码的中文学习实验室。它不是提醒事项业务 App，而是一台“UIKit 显微镜”：用紧凑实验控制台、结构化日志、Xcode Call Stack 和 UI Test，观察 UIKit 什么时候自动调用生命周期、delegate、target-action、closure 回传、snapshot 刷新和 cell 复用。
...
1. App 中的 `GuidedStep` 只保留当前动作、观察位置、真实源码入口和一个 Xcode 操作。
2. `DemoLogStore` 同时保存结构化事件并输出 Xcode Console。
3. App 内 Log Panel 支持分类、搜索、关键事件、暂停滚动与复制。
4. Call Stack 回答“谁触发”，UI Test 检查关键用户流程。

详细预测题、预期日志、机制解释和复盘全部放在 `docs/guided-learning.md`；App 不再充当教材阅读器。九张独立实验卡都包含真实源码锚点、App 操作、Xcode / LLDB 操作、证据、Reset、边界和思考题，并由 `scripts/audit-project.py` 与 catalog 逐项校验。
...
1. 运行 App。
2. 点右上角 `Learn -> Guide`。
3. Guide 一次只显示一个实验动作；用 `⌘⇧O` 打开它指向的真实源码。
...
Simulator: iPhone 17 Pro + 当前 Xcode 的 iOS Simulator SDK
```

Makefile 会识别仓库路径迁移并清理旧 DerivedData，避免旧 checkout 的绝对路径污染当前构建。
